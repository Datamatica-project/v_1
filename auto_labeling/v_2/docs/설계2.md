...
8. [설정 및 배포](#8-설정-및-배포)
   - 8.1 [주요 설정 파일](#81-주요-설정-파일)
   - 8.2 [배포 구조](#82-배포-구조)
   - 8.3 [Round 반복 전략](#83-round-반복-전략-및-종료-조건)
9. [마이그레이션 계획](#9-마이그레이션-계획)
   - 9.1 [v_1에서 v_2로 전환](#91-fastapi-api-서버)
   - 9.2 [v_1과의 차이점](#92-v_1과의-차이점)
10. [확장 및 개선 방향](#10-확장-및-개선-방향)
11. [참고 자료](#11-참고-자료)
...
```
1. GT 데이터 업로드
   └─→ POST /api/v2/gt/ingests/upload (ZIP 업로드)
   └─→ POST /api/v2/gt/ingests/{id}/register (GT 등록)
        ├─ YOLO 형식 지원 (images/, labels/)
        ├─ COCO JSON 형식 지원 (images/, annotations/)
        └─ 자동 형식 감지

2. Unlabeled 이미지 업로드
   └─→ POST /api/v2/unlabeled/ingests/upload (이미지 ZIP)

3. Loop 실행
   └─→ POST /api/v2/loop/start
        └─ Worker가 백그라운드에서 Loop 시작
        └─ loopId, runId 즉시 반환

4. 진행 상태 폴링 (Spring Boot)
   └─→ GET /api/v2/events/status?runId=xxx (2~3초마다)
        └─ 상태 파일 읽기: logs/events/{runId}/status.json
        └─ 응답: {status, rounds, currentRound, updatedAt}

5. Round 완료 시 Export (Spring Boot가 수동 호출)
   └─→ POST /api/v2/export/round (Round 0, 1, 2 완료 시)
   └─→ GET /api/v2/export/download (ZIP 다운로드)

6. Loop 완료 시 Final Export
   └─→ POST /api/v2/export/final
   └─→ GET /api/v2/export/download
        └─ 모든 Round의 PASS 통합 + 최종 FAIL/MISS
```
...
│  └── logs/                                                     │
│      ├── events/{runId}/    (Loop 상태 및 로그)               │
│      │   └── status.json    (Loop 진행 상태 - 단일 파일)     │
...
└──────┬──────┘
       ...
       │ 폴링 루프 (2~3초)
       │
       │ GET /api/v2/events/status?runId=xxx
       ▼
┌─────────────┐
│  API Server │ → logs/events/<runId>/status.json 파일 읽기
└──────┬──────┘
       │
       │ Loop 상태 반환 (status, rounds, currentRound, etc.)
       ▼
┌─────────────┐
│ Spring Boot │ → React UI 업데이트 (진행률, Round 상태 등)
└─────────────┘
```
...
Worker Loop 실행
    │
    ├─ Round 0 시작 → status.json 업데이트 (round0: RUNNING)
    ├─ Round 0 완료 → status.json 업데이트 (round0: DONE, round1: RUNNING)
    ├─ Round 1 완료 → status.json 업데이트 (round1: DONE, round2: RUNNING)
    └─ Loop 완료   → status.json 업데이트 (status: COMPLETED)
```
...
│       ├── events_worker.py         # 이벤트 로그 기록 클라이언트
...
```http
POST /api/v2/gt/ingests/upload
Content-Type: multipart/form-data

Parameters:
- file: ZIP 파일 (images/, labels/, annotations/ 포함)
...
Response: 200 OK
{
  ...
  "next": "/api/v2/gt/ingests/{ingestId}/register"
}
```

**지원 형식:**
- **YOLO 형식**: `images/`, `labels/` (txt 파일, `class cx cy w h`)
- **COCO JSON 형식**: `images/`, `annotations/` (instances_train.json, instances_val.json)
- 혼합 형식도 지원 (YOLO txt + COCO JSON 동시 포함 가능)
...
```http
POST /api/v2/gt/ingests/{ingest_id}/register
Query Parameters:
...
**처리 과정:**
1. `extracted/images` 검증
2. 형식 자동 감지:
   - `annotations/` 폴더 있으면 → COCO JSON
   - `labels/` 폴더 있으면 → YOLO txt
   - 둘 다 있으면 → MIXED
3. `GT_versions/GT_{ingestId}`로 복사 또는 심볼릭 링크
4. 라벨 형식 검증:
   - YOLO: `class cx cy w h`
   - COCO: instances_train.json, instances_val.json 스키마 검증
5. `data/GT` 심볼릭 링크를 최신 버전으로 갱신
6. `data.yaml` 생성 (클래스 정보 포함)

**파일:** `auto_labeling/v_2/api/routers/ingest.py`, `auto_labeling/v_2/src/gt_register.py`
...
```http
POST /api/v2/gt/convert
Content-Type: application/json

Request Body:
{
  "ingestId": "gt_20250106_120000_abc123",
  "sourceFormat": "COCO",  // COCO | YOLO
  "targetFormat": "YOLO"   // COCO | YOLO
}
...
Response: 200 OK
{
  ...
  "data": {
    "ingestId": "gt_20250106_120000_abc123_converted",
    "sourceFormat": "COCO",
    "targetFormat": "YOLO",
    "convertedPath": ".../GT_versions/GT_gt_xxx_yolo"
  }
}
```
...
- 변환된 데이터는 새로운 ingestId로 저장

**파일:** `auto_labeling/v_2/api/routers/convert.py` (향후 구현)
...
4. Worker는 Loop 진행 중 이벤트를 로그 파일에 직접 기록 (`logs/events/<runId>/<timestamp>_<eventType>.json`)
...
**처리:**
- Worker가 기록한 `logs/events/<runId>/status.json` 파일 읽기
- 단일 상태 파일 방식으로 최신 상태 반환
- Worker는 Loop 진행 중 status.json 파일을 계속 업데이트
...
public void pollLoopStatus() {
    String url = apiBaseUrl + "/api/v2/events/status?runId=" + runId;
...
Response: 200 OK
{
  ...
  "data": [
    {
      "runId": "run_20250107_103000_xyz789",
...
**플로우:**
1. Spring Boot가 `/api/v2/events/status` 폴링으로 Round 상태 체크
2. Round 완료 확인 시 (`round0: DONE`) Spring Boot가 Export API 호출
3. API Server가 해당 Round 결과를 ZIP으로 압축하여 반환

---

#### 1) Round별 Export 생성 (BE 수동 호출)

```http
POST /api/v2/export/round
Content-Type: application/json
...
Request Body:
{
  "loopId": "loop_abc123",
  "round": 0,
  "format": "YOLO"  // YOLO | COCO (선택, 기본값: YOLO)
}

Response: 200 OK
{
  "resultCode": "SUCCESS",
  "message": "Round export completed",
  "data": {
    "loopId": "loop_abc123",
    "round": 0,
    "format": "YOLO",
    "exportPath": "data/exports/loop_abc123/run_0.zip",
    "downloadUrl": "/api/v2/export/download?loopId=loop_abc123&type=round&round=0",
...
**처리:**
- `data/results/loop_{loop_id}/run_{round}/` 디렉토리를 지정된 형식으로 압축
- `data/exports/{loop_id}/run_{round}.zip` 에 캐시 (멱등성)
- 이미 존재하면 재생성하지 않음

---

#### 2) Final Export (전체 통합, BE 수동 호출)
...
Response: 200 OK
{
  ...
  "data": {
    "loopId": "loop_abc123",
    "format": "YOLO",
    "exportPath": "data/exports/loop_abc123/final.zip",
    "downloadUrl": "/api/v2/export/download?loopId=loop_abc123&type=final",
    "fileSize": 52428800,
...
- 최종 Round의 `FAIL/`, `MISS/` 포함
- `data/exports/{loop_id}/final.zip` 에 캐시
- 중복 이미지 제거 (runId + fileName 기준)
...
GET /api/v2/export/download
Query Parameters:
- loopId: loop_abc123 (필수)
- type: round | final (필수)
- round: 0 (type=round일 때 필수)

Response: 200 OK (ZIP 파일 스트리밍)
Content-Type: application/zip
Content-Disposition: attachment; filename="loop_abc123_round_0.zip"
```

**처리:**
- 캐시된 ZIP 파일 반환
- 캐시가 없으면 404 (먼저 POST로 생성 필요)

---

#### 4) Spring Boot 플로우 예시

```java
// Round 완료 감지
@Scheduled(fixedDelay = 2000)
public void pollLoopStatus() {
    StatusResponse status = getLoopStatus(runId);

    // Round 0 완료 감지
    if (status.getRounds().get("round0").equals("DONE")
        && !exportedRounds.contains(0)) {

        // Export 요청
        ExportRequest req = new ExportRequest("loop_abc123", 0, "YOLO");
        ExportResponse res = exportRound(req);

        // 다운로드 URL 저장
        saveDownloadUrl(res.getDownloadUrl());
        exportedRounds.add(0);
    }

    // Loop 완료 시 Final Export
    if (status.getStatus().equals("COMPLETED")) {
        ExportRequest req = new ExportRequest("loop_abc123", "YOLO");
        ExportResponse res = exportFinal(req);
        saveDownloadUrl(res.getDownloadUrl());
    }
}
```
...
│                                                             │
│ 이벤트 로그 기록:                                           │
│   logs/events/run_xyz789/{timestamp}_ROUND_COMPLETED.json  │
│   {                                                         │
...
│                                                             │
│ 이벤트 로그 기록: ROUND_COMPLETED (failMissRatio: 0.08)    │
└─────────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
...
def run_ensemble_loop(
    *,
    ...
    for round_idx in range(max_rounds):
        ...
        # 이벤트 로그 기록
        write_event_log("ROUND_COMPLETED", run_id, round_result)
...
### 7.3 Worker 이벤트 로그 기록 (단일 상태 파일)

**파일:** `auto_labeling/v_2/worker/services/status_logger.py`

```python
def update_loop_status(
    run_id: str,
    status: str = "RUNNING",
    rounds: Dict[str, str] = None,
    current_round: Optional[int] = None,
    total_rounds: Optional[int] = None,
    **extra_data
):
    """
    Loop 상태를 단일 파일에 기록 (덮어쓰기)

    저장 경로: logs/events/{run_id}/status.json

    Args:
        run_id: Run 식별자
        status: Loop 상태 (RUNNING, COMPLETED, FAILED)
        rounds: Round별 상태 ({"round0": "DONE", "round1": "RUNNING", ...})
        current_round: 현재 Round 번호
        total_rounds: 전체 Round 수
        **extra_data: 추가 데이터
    """
    ...
    events_dir = Path("logs/events") / run_id
    ...
    status_data = {
        "status": status,
        "rounds": rounds or existing_data.get("rounds", {}),
        "updatedAt": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    }

    if current_round is not None:
        status_data["currentRound"] = current_round
    if total_rounds is not None:
        status_data["totalRounds"] = total_rounds
...
update_loop_status(
    run_id=run_id,
    status="RUNNING",
    rounds={"round0": "RUNNING", "round1": "PENDING", "round2": "PENDING"},
    current_round=0,
    total_rounds=3
)

# Round 0 완료, Round 1 시작
update_loop_status(
    run_id=run_id,
    status="RUNNING",
    rounds={"round0": "DONE", "round1": "RUNNING", "round2": "PENDING"},
    current_round=1,
    total_rounds=3
)
...
update_loop_status(
    run_id=run_id,
    status="COMPLETED",
    rounds={"round0": "DONE", "round1": "DONE", "round2": "DONE"},
    current_round=2,
    total_rounds=3
)
...
#### 8.1.1 Ensemble Loop 설정 (`configs/ensemble_config.yaml`)
...
#### 8.1.2 클래스 정의 (`configs/classes.yaml`)
